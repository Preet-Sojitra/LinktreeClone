      ...
      <div className="flex gap-2 items-center">
        {data.socials.map((link) => {
          if (link.url.includes("twitter")) {
            return <TwitterIcon key={link.url} />
          }
          if (link.url.includes("github")) {
            return <GitHubIcon key={link.url} />
          }
        })}
      </div>
    </div>
...
function LinkCard({title, url, image}) {
  return (
    <a
      href={url}
      className="flex items-center p-1 rounded-md hover:scale-105 transition-all duration-300 w-full bg-gray-100 mb-3 max-w-3xl"
    >
      <div className="flex w-full text-center">
        ...
        <h2 className="font-semibold flex justify-center items-center w-full text-gray-700 -ml-10 ">
          {title}
        </h2>
      </div>
    </a>
